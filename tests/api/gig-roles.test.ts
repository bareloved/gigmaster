...
describe("Gig Roles API", () => {
  ...
  describe("updateRole", () => {
    it("should update role and return updated data", async () => {
      // First call: fetch current role, second call: update
      let callCount = 0;
    ...
    it("should notify musician when payment status changes to paid", async () => {
        ...
        if (callCount === 1) {
          return createChainableMock({
            data: {
              ...
              role_name: "Bass",
              gigs: { id: "gig-1", title: "Jazz Night" },
            },
            error: null,
          });
        }
        return createChainableMock({
          data: { ...mockGigRole, payment_status: "paid" },
          error: null,
        });
      });

      await updateRole("test-role-id", { payment_status: "paid" });

      expect(createNotification).toHaveBeenCalledWith(
        expect.objectContaining({
          user_id: "musician-123",
