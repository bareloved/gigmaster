...
export async function upsertLearningStatus(
  ...
  return {
    ...
    difficulty: result.difficulty as 'easy' | 'medium' | 'hard' | null,
...
export async function getMusicianLearningStatuses(
  ...
  return data.map(item => ({
    ...
    difficulty: item.difficulty as 'easy' | 'medium' | 'hard' | null,
...
export async function getPracticeItems(
  ...
  for (const item of setlistData) {
    ...
    practiceItems.push({
      ...
      gigTitle: gig.title,
      gigDate: gig.date,
      hostName: Array.isArray(gig?.owner) ? gig.owner[0]?.name || null : gig?.owner?.name || null,
      key: item.key,
      bpm: item.tempo ? parseInt(item.tempo, 10) : null, // Parse tempo string to number for bpm
