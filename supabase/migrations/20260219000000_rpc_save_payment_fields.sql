-- Migration: Update save_gig_pack RPC to persist payment fields on gig_roles
-- Purpose: Allow payment data (agreed_fee, currency, payment_method, expected_payment_date)
--          to be set before the gig is saved, and carried through the RPC save flow.
--
...
BEGIN
  ...
  -- Now includes payment fields: agreed_fee, currency, payment_method, expected_payment_date
  -- UPDATE uses CASE guards so payment is only overwritten when the key exists in JSON
  -- ========================================
  BEGIN
    IF jsonb_array_length(p_roles) > 0 THEN
      IF p_is_editing THEN
        -- Update existing roles (matched by gigRoleId)
        UPDATE gig_roles gr SET
          role_name = item->>'role',
          musician_name = item->>'name',
          notes = item->>'notes',
          -- Only overwrite payment fields if the key is present in JSON
          agreed_fee = CASE WHEN item ? 'agreedFee' THEN (item->>'agreedFee')::numeric ELSE gr.agreed_fee END,
          currency = CASE WHEN item ? 'currency' THEN COALESCE(item->>'currency', gr.currency) ELSE gr.currency END,
          payment_method = CASE WHEN item ? 'paymentMethod' THEN item->>'paymentMethod' ELSE gr.payment_method END,
          expected_payment_date = CASE WHEN item ? 'expectedPaymentDate' THEN (item->>'expectedPaymentDate')::date ELSE gr.expected_payment_date END
        FROM jsonb_array_elements(p_roles) AS item
        WHERE gr.id = (item->>'gigRoleId')::uuid
          AND gr.gig_id = v_gig_id;

        -- Insert new roles added while editing
        INSERT INTO gig_roles (gig_id, role_name, musician_name, musician_id, contact_id, notes, sort_order, invitation_status,
                               agreed_fee, currency, payment_method, expected_payment_date)
        SELECT
          v_gig_id,
          item->>'role',
          item->>'name',
          ...
          CASE
            ...
            ELSE 'pending'
          END,
          (item->>'agreedFee')::numeric,
          COALESCE(NULLIF(item->>'currency', ''), 'ILS'),
          NULLIF(item->>'paymentMethod', ''),
          (NULLIF(item->>'expectedPaymentDate', ''))::date
      ...
      ELSE
        -- Insert all roles for a new gig
        INSERT INTO gig_roles (gig_id, role_name, musician_name, musician_id, contact_id, notes, sort_order, invitation_status,
                               agreed_fee, currency, payment_method, expected_payment_date)
        SELECT
          v_gig_id,
...
COMMENT ON FUNCTION save_gig_pack IS 'Atomic RPC to save entire gig pack (gig + schedule, materials, packing, setlist, roles, contacts, shares) in a single transaction. Roles now include payment fields.';
