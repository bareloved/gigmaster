-- Migration: Fix creator invitation status in save_gig_pack RPC
-- Problem: When a gig creator adds themselves to the lineup, they get
--   invitation_status = 'invited' — same as any other musician. This causes
--   the "Invited" badge to appear on their own gig, which is confusing.
-- Fix: Check if the lineup member's user ID matches the gig creator (v_user_id).
--   If so, set invitation_status = 'accepted' instead of 'invited'.
-- Also backfills existing data.
...
BEGIN
  ...
  -- Creator auto-accepted: If the lineup member's user ID matches the
  -- gig creator (v_user_id), set invitation_status = 'accepted' instead
  -- of 'invited'. The creator shouldn't need to accept their own gig.
  -- ========================================
  BEGIN
    ...
    IF jsonb_array_length(p_roles) > 0 THEN
      IF p_is_editing THEN
        ...
        SELECT
          ...
          CASE
            WHEN COALESCE(NULLIF(item->>'userId', ''), NULLIF(item->>'linkedUserId', ''))::uuid = v_user_id
            THEN 'accepted'
...
COMMENT ON FUNCTION save_gig_pack IS 'Atomic RPC to save entire gig pack (gig + schedule, materials, packing, setlist, roles, contacts, shares) in a single transaction. Creator auto-accepted in lineup.';

-- ========================================
-- Backfill: Fix existing gig_roles where the creator is marked as 'invited'
-- ========================================
UPDATE gig_roles gr
SET invitation_status = 'accepted'
FROM gigs g
WHERE gr.gig_id = g.id
  AND gr.musician_id = g.owner_id
