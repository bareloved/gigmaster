  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(`/settings?tab=calendar&error=oauth_${error}`, request.url)
      );
    }

    if (!code) {
      return NextResponse.redirect(
        new URL("/settings?tab=calendar&error=missing_code", request.url)
      );
    }
    ...
    if (dbError) {
      ...
      return NextResponse.redirect(
        new URL("/settings?tab=calendar&error=save_failed", request.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL(`/settings?tab=calendar&success=${successParam}`, request.url)
    );
  } catch (error: unknown) {
    console.error("OAuth callback error:", error);
    return NextResponse.redirect(
      new URL(`/settings?tab=calendar&error=callback_failed`, request.url)
    );
  }
}
